import processBlockquotes from "@/components/helpers/markdown/processBlockquotes";
import processCodeBlocks from "@/components/helpers/markdown/processCodeBlocks";
import processLists from "@/components/helpers/markdown/processLists";
import processTables from "@/components/helpers/markdown/processTables";
...
function parseMarkdown(content: string): string {
  ...
  processedContent = processedContent.replace(
    ...
    (match, content) => {
      ...
      codeSpans.push(
        `<code class="bg-muted px-1 py-0.5 rounded text-xs font-mono">${content}</code>`,
      );
      codeSpanCounter++;
      return placeholder;
    },
  ...
  codeBlocks.forEach((codeBlock, index) => {
    processedContent = processedContent.replace(`CODEBLOCK${index}`, codeBlock);
  });
