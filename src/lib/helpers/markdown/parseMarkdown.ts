...
function parseMarkdown(content: string): string {
  ...
  processedContent = processedContent.replace(/^\[\^([^\]]+)\]:\s*(.+)$/gm, (match, id, definition) => {
    footnotes.set(id, definition.trim());
    return '';
  });
  
  processedContent = processedContent.replace(/\[\^([^\]]+)\]/g, (match, id) => {
    if (footnotes.has(id)) {
  ...
  /**
   * Sequence: Process the inline elements
   */
  processedContent = processedContent
    ...
    .replace(/  $/gm, '<br />')
    .replace(/\[\[([^\]]+)\]\]/g, '<kbd class="px-1.5 py-0.25 text-xs font-mono bg-muted border border-border rounded">$1</kbd>');
  ...
  if (footnoteRefs.length > 0) {
    let footnotesHtml = '<div class="footnotes mt-8 pt-4 border-t border-border/50"><h4 class="text-sm font-medium mb-3">Footnotes</h4><ol class="text-xs space-y-2">';
